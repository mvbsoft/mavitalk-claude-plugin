...
case "$prompt" in
  *завершу*|*закінчу*|*заверша*|*finish*|*"wrap up"*|*"wrap it up"*|*"done for today"*|*"closing the session"*)
    emit "The user is signalling the END of the coding session. Invoke the mavitalk:finishing-the-session skill."
    exit 0 ;;
esac
case "$prompt" in
  *продовж*|*почнемо*|*почина*|*continue*|*resume*|*"pick up where"*)
    emit "The user is RESUMING work. Invoke the mavitalk:continue-session skill."
    exit 0 ;;
esac
exit 0
